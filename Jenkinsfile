                ...
                stage('Deploy To Kubernetes') {
                        steps {
                                withKubeConfig(caCertificate: '', clusterName: 'abrahimcse-cluster', contextName: '', credentialsId: 'k8-cred', namespace: 'webapps', restrictKubeConfigAccess: false, serverUrl: 'https://< >.ap-southes-1.eks.amazonaws.com') {
                                            // prefer Helm if available, fallback to kubectl apply
                                            sh '''
                                                if command -v helm >/dev/null 2>&1; then
                                                    helm upgrade --install bloggingapp ./k8s/helm/FullStack-Blogging-App -n webapps --set image.repository=abrahimcse/bloggingapp --set image.tag=latest
                                                else
